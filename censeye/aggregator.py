...
class Aggregator:
    ...
    def _generate_ldap_queries(self, data, parent_key):
        results = []
            ...
            if "values" in attr:
                for val in attr["values"]:
                    ...
                    results.append(
                        (
                            parent_key,
                            f"ldap.attributes.name='{attr['name']}' and ldap.attributes.values: '{val}'",
                        )
                    )
        return results
    ...
    def _generate_queries(self, data, parent_key=""):
        ...
        if isinstance(data, dict):
            for k, v in data.items():
                ...
                elif key == "services.ldap.attributes":
                    # special case for ldap attributes
                    results.extend(self._generate_ldap_queries(v, key))
                else:
    ...
    async def _get_aggregate_report(self, key, value):
        ...
        if key.startswith("~"):
            ...
            query = f"services: ({value})"
        elif key.endswith(f".{GADGET_NAMESPACE}") or key == "services.ldap.attributes":
